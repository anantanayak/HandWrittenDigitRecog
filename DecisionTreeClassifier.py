...
for (dirname, dirs, files) in os.walk(path):
        for filename in files:
            if filename.endswith('.jpg'):        
                ...
                img_gray = cv2.GaussianBlur(img_gray, (15,15),0)                 #Gaussian filter to reduce noise and improve edge detection     
...
clf = clf.fit(Bi_Train,labelsTrain)
end = time.time()

#Testing the model with Test images
predict = clf.predict(Bi_Test)
